            ...
            {missions.map(m => {
              ...
              return (
                <Link key={m.id} href={`/missions/${m.id}`}
                  className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all border border-gray-100 hover:border-purple-200">
                  {/* 카드 상단 컬러 배너 */}
                  ...
                  <div className="p-5">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <span className="text-lg font-extrabold text-purple-600">{Number(m.rewardAmount).toLocaleString()}원</span>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-3">
                      <span className="text-xs bg-gray-50 text-gray-500 px-2 py-1 rounded-lg">📌 {m.regionSi} {m.regionGu}</span>
                      <span className="text-xs bg-gray-50 text-gray-500 px-2 py-1 rounded-lg">👥 {m.maxParticipants}명 모집</span>
                      <span className="text-xs bg-gray-50 text-gray-500 px-2 py-1 rounded-lg">📅 {new Date(m.endDate).toLocaleDateString('ko-KR', { month: 'short', day: 'numeric' })} 마감</span>
                    </div>
                    {/* 신청 현황 바 */}
                    <div>
          ...
          <div className="space-y-3">
            {submissions.map(s => (
              <div key={s.id} className="bg-white rounded-2xl p-5 shadow-sm">
                ...
                {s.description && <p className="text-sm text-gray-600 bg-gray-50 rounded-lg p-3 mt-2">{s.description}</p>}
            ...
            <form onSubmit={handleWritePost} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <div className="border rounded-lg px-3 py-2 text-sm bg-gray-50 text-gray-400">{user.name}</div>
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <textarea required rows={7} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-purple-500 focus:outline-none resize-none"
                  value={writeForm.content} onChange={e => setWriteForm(f => ({ ...f, content: e.target.value }))} placeholder="내용을 입력하세요" />
              </div>
              <div className="flex justify-end gap-2">
                <button type="button" onClick={() => setShowWriteModal(false)} className="px-4 py-2 border rounded-lg text-sm text-gray-500 hover:bg-gray-50">취소</button>
                <button type="submit" disabled={submittingPost} className="px-6 py-2 bg-purple-600 text-white rounded-lg text-sm font-bold hover:bg-purple-700 disabled:opacity-50">
