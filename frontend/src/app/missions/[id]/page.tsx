...
function InfoRow({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex gap-0 border-b border-gray-50 last:border-0">
      <div className="w-32 shrink-0 py-3.5 px-4 text-sm text-gray-500 font-medium bg-stone-50">{label}</div>
...
export default function MissionDetailPage() {
  ...
  if (!mission) return (
    <div className="min-h-screen flex items-center justify-center bg-stone-50 text-gray-400">미션을 찾을 수 없습니다</div>
  );
  ...
  return (
    <div className="min-h-screen bg-gray-100 pb-20 lg:pb-0">
      ...
      <nav className="bg-white border-b px-4 sm:px-6 py-3 flex items-center gap-3 sticky top-0 z-20 shadow-sm">
        ...
        <h1 className="text-base font-bold text-slate-800">맘브릿지</h1>
      </nav>
      ...
      <div className="bg-white border-b px-4 sm:px-6 py-4">
        <div className="max-w-5xl mx-auto">
          <div className="flex items-start gap-3 flex-wrap mb-2">
            ...
            <span className="ml-auto text-xl font-extrabold text-indigo-600">{Number(mission.rewardAmount).toLocaleString()}원</span>
          </div>
        ...
        <div className="flex-1 min-w-0 space-y-4">
          ...
          <div className="bg-white rounded-xl overflow-hidden shadow-sm">
            {mission.thumbnailUrl ? (
              ...
              <div className="w-full h-44 bg-gradient-to-br from-indigo-50 to-pink-50 flex flex-col items-center justify-center">
                <div className="text-5xl mb-2">{{ VISIT: '📍', SNS: '📸', REVIEW: '✍️', VIDEO: '🎬' }[mission.category as string] ?? '🎯'}</div>
                <p className="text-indigo-400 text-sm font-medium">{mission.clientProfile?.businessName}</p>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-5">
            <p className="text-sm font-bold text-gray-700 mb-1">해당 체험단은 <span className="text-indigo-600">{CATEGORY_LABEL[mission.category]}</span> 체험단입니다.</p>
            ...
            <div className="space-y-2">
              {[
                ...
                <div key={step} className="flex gap-2 text-sm text-gray-600">
                  <span className="shrink-0 font-bold text-indigo-500">{step}</span>
                  <span>{text}</span>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white rounded-xl shadow-sm overflow-hidden">
            ...
            <InfoRow label="제공서비스/물품">
              <span className="font-semibold text-indigo-700">{Number(mission.rewardAmount).toLocaleString()}원</span>
          ...
          <div className="bg-white rounded-xl shadow-sm p-5">
            <h3 className="text-sm font-bold text-gray-800 mb-3 flex items-center gap-2">
              <span className="w-1 h-4 bg-indigo-500 rounded-full" />미션 소개
            </h3>
          ...
          <div className="bg-white rounded-xl shadow-sm p-5">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {mission.category === 'SNS' && ['#맘브릿지', '#체험단'].map(t => (
                <span key={t} className="text-xs bg-indigo-50 text-indigo-600 px-3 py-1.5 rounded-full font-medium">{t}</span>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-5">
            ...
            <div className="grid grid-cols-4 gap-3">
              {missionIcons.map(({ icon, label }) => (
                <div key={label} className="flex flex-col items-center gap-2 bg-stone-50 rounded-xl py-3 px-2">
          ...
          <div className="bg-white rounded-xl shadow-sm p-5">
            ...
            {user && (
              <div className="border-t pt-4 flex gap-2">
                ...
                />
                <button className="px-4 py-2 bg-indigo-700 text-white rounded-lg text-sm font-medium hover:bg-indigo-800">등록</button>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="hidden lg:block w-72 shrink-0 sticky top-16 space-y-4">
          ...
          <div className="bg-white rounded-xl shadow-sm overflow-hidden">
            ...
            <div className="p-4 space-y-1">
              <DateRange label="신청 기간" start={mission.startDate} end={mission.endDate} color="bg-indigo-100 text-indigo-700" />
            ...
            {/* D-Day 카운터 */}
            <div className="mx-4 mb-4 bg-stone-50 rounded-lg py-3 text-center">
              {dday > 0 ? (
                <>
            ...
            <div className="px-4 pb-4">
              {user?.role === 'PARTICIPANT' && isOpen && !isFull && !applied && (
                <div className="space-y-2">
                  <textarea
                    className="w-full border rounded-lg px-3 py-2 text-xs resize-none focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    rows={3} placeholder="지원 동기를 간단히 적어주세요 (선택)"
                    value={applyMsg} onChange={e => setApplyMsg(e.target.value)}
                  />
                  <button onClick={handleApply} disabled={applying}
                    className="w-full bg-indigo-700 text-white rounded-lg py-3 font-bold text-sm hover:bg-indigo-700 disabled:opacity-50 shadow-md">
                    {applying ? '신청 중...' : '신청하기'}
                  </button>
                </div>
              )}
              ...
              {!user && (
                <Link href="/login" className="block w-full bg-indigo-700 text-white rounded-lg py-3 font-bold text-sm text-center hover:bg-indigo-700 shadow-md">
                  로그인 후 신청하기
                </Link>
              )}
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t shadow-lg px-4 py-3 z-30">
        <div className="flex items-center gap-3">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-base font-bold text-indigo-600">{Number(mission.rewardAmount).toLocaleString()}원</p>
          </div>
          <div className="shrink-0 w-44">
            {user?.role === 'PARTICIPANT' && isOpen && !isFull && !applied && (
              <button onClick={handleApply} disabled={applying}
                className="w-full bg-indigo-700 text-white rounded-xl py-2.5 font-bold text-sm hover:bg-indigo-700 disabled:opacity-50">
                {applying ? '신청 중...' : '신청하기'}
              </button>
            )}
            {applied && (
            ...
            {!user && (
              <Link href="/login"
                className="block w-full bg-indigo-700 text-white rounded-xl py-2.5 font-bold text-sm text-center hover:bg-indigo-800">
                로그인 후 신청하기
              </Link>
            )}
            {(user?.role === 'CLIENT' || user?.role === 'OPERATOR') && (
